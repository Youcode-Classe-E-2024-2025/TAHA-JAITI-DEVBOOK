...
const initDB = async () => {
    await checkConn().then(async () => {
        try {
            ...
            try {
                await fs.promises.access('/app/src/db/script.sql');
            } catch (err) {
                console.error("SQL file not found at:", '/app/src/db/script.sql');
                console.error("Current working directory:", process.cwd());
                console.error("Directory contents:", await fs.promises.readdir('/app/src/db'));
                throw err;
            }
            ...
            if (tableCount === 0) {
                ...
                const sql = fs.readFileSync('/app/src/db/script.sql', 'utf8');
